...
| Custom attributes sidecar | Per-edge multi-attribute float overlay (`custom_attributes.tar`). Format: `valhalla/baldr/custom_attributes_tile.h` (header: `edge_count` + `num_attributes`, then row-major float array). The tar contains `attributes.json` (JSON array of names in column order) — `GraphReader` reads it once at startup via `GetCustomAttributeNames()`. Build script: `scripts/build_custom_attributes_tar.py --attribute-names name1,name2`. Config key: `mjolnir.custom_attributes_extract` (tar path only — names come from the tar). Costing param: `use_custom_attributes: {"name": weight}` map in `Costing.Options`. Names-to-index mapping flows: `GraphReader` reads `attributes.json` → `loki/worker.cc` injects into `Costing.custom_attribute_names` proto field → `DynamicCost` constructor stores in `custom_attribute_names_` → `AutoCost` builds dense weight vector. Response field: `edge.custom_attributes` map in `trace_attributes`. Full guide: `docs/docs/custom_attributes.md` |
