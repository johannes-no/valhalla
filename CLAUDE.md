...
| Custom attributes sidecar | Per-edge multi-attribute float overlay (`custom_attributes.tar`). Format: `valhalla/baldr/custom_attributes_tile.h` (header: `edge_count` + `num_attributes`, then row-major float array). Build script: `scripts/build_custom_attributes_tar.py --num-attributes N`. Config keys: `mjolnir.custom_attributes_extract` (tar path) and `mjolnir.custom_attributes_names` (ordered name list). Costing param: `use_custom_attributes: {"name": weight}` map in `Costing.Options`. Loaded by `GraphReader`; attached to `GraphTile` via `custom_attributes_tile()`. Names-to-index mapping flows: `ParseAutoCostOptions` reads `mjolnir.custom_attributes_names` from doc → writes to `Costing.custom_attribute_names` proto field → `DynamicCost` constructor stores in `custom_attribute_names_` → `AutoCost` builds dense weight vector. Response field: `edge.custom_attributes` map in `trace_attributes`. Full guide: `docs/docs/custom_attributes.md` |
